...
@IBDesignable final class ButtonCell: NSButtonCell {
    ...
    override func drawBezel(withFrame frame: NSRect, in controlView: NSView) {
        ...
        let outerClip = NSBezierPath(roundedRect: frame, xRadius: roundedRadius, yRadius: roundedRadius)
        outerClip.setClip()

        let outerGradient = NSGradient(colorsAndLocations: (NSColor(deviceWhite: 0.20, alpha: 1.0), 0.0), (NSColor(deviceWhite: 0.21, alpha: 1.0), 1.0))
        outerGradient?.draw(in: outerClip.bounds, angle: 90.0)
        ctx.restoreGraphicsState()

        // Background gradient
        ctx.saveGraphicsState()
        let backgroundPath: NSBezierPath = NSBezierPath(roundedRect: NSInsetRect(frame, 2.0, 2.0), xRadius: roundedRadius, yRadius: roundedRadius)
        ...
        if isHighlighted {
            ctx.saveGraphicsState()
            NSBezierPath(roundedRect: NSInsetRect(frame, 2.0, 2.0), xRadius: roundedRadius, yRadius: roundedRadius).setClip()
